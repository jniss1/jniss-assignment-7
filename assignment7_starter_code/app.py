...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    Y = np.random.normal(mu, np.sqrt(sigma2), N)   # Replace with code to generate Y
    ...
    plt.plot(X, model.predict(X.reshape(-1,1)), color="red",label=f"Fitted Line: Y={slope:.2f}X+{intercept:.2f}")
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.title("Scatter Plot of Generated Data with Fitted Regression Line")
    plt.legend()
...
def hypothesis_test():
    ...
    if parameter == "slope":
        ...
        # Used for labeling in the chart
        label_beta_type = r"$\beta_1"
    else:
        simulated_stats = np.array(intercepts)
        observed_stat = intercept
        hypothesized_value = beta0
        label_beta_type = r"$\beta_0"
    ...
    if p_value is not None and p_value <= 0.0001:
        fun_message = "(T o T) It's beutiful!"
    else:
        fun_message = None
    ...
    plt.hist(simulated_stats, bins=30, color='green', edgecolor=None, alpha=0.7, label="Simulated Statistics")
    plt.axvline(observed_stat, color='purple', linestyle='dashed', linewidth=1,
                label=f'Observed {parameter.capitalize()} ({observed_stat:.4f})')
    plt.axvline(hypothesized_value, color='pink', linestyle=':', linewidth=1,
                label=f'Hypothesized {parameter.capitalize()} ({label_beta_type} = {hypothesized_value:.4f})')
...
def confidence_interval():
    ...
    ci_lower = mean_estimate - stats.t.ppf(1 - alpha / 2, df=S - 1) * (std_estimate / np.sqrt(S))
    ci_upper = mean_estimate + stats.t.ppf(1 - alpha / 2, df=S - 1) * (std_estimate / np.sqrt(S))
    ...
    plt.scatter(estimates, np.zeros_like(estimates), color='blue', alpha=0.5, label='Simulated Estimates')
    plt.scatter(mean_estimate, 0, color='blue', s=100, label='Mean Estimate for Given beta:')
    ci_color = 'green' 
    plt.hlines(0, ci_lower, ci_upper, colors=ci_color, linewidth=4, label=f'{confidence_level}% Confidence Interval')
    plt.axvline(true_param, color='red', linestyle=':', linewidth=2, label=f'True Beta: ')
    plt.yticks([])
    plt.ylabel('')
